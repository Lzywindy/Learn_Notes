...
class LLM_Inference(object):
    def __init__(self,name_or_path:str,device: Optional[str] ,dtype: torch.dtype ,**karges) -> None:
        ...
        lora_config=karges.get("lora_config",None)
        if isinstance(lora_config,dict):
            path=lora_config.get("path",None)
            assert path and os.path.exists(path)
            lora_head=lora_config.get("lora_head","default")
            lora_config = LoraConfig(
                ...
                target_modules=lora_config.get("lora_targe",["q_proj", "k_proj", "v_proj", "o_proj"]),
                modules_to_save=lora_config.get("additional_target", None),
            )
            model = get_peft_model(model, lora_config, lora_head)
            print("从`{}`加载lora头".format(path))
            model.load_adapter(path,lora_head)
